...
Generates realistic synthetic source files in `01_Sources/` to test the full pipeline (`/analyze` → `/synthesis` → `/ship`) without needing real project data. Also useful as onboarding — a new user runs `/seed` and sees the system working in minutes.
