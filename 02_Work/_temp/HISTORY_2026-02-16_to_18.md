...
### Branding + Footer + PDF exports
- Footer bar: theme toggle (izq), logo Idemax (centro), nav (der). Reemplaza floating controls.
- Slide 34: cierre Idemax (#f93545 bg, logo blanco vía brightness(0) invert(1)).
- Header compactado: slide-title 20px, max-height 32px.
- content-grid centrado: eliminados todos los .content-grid--top y inline overrides.
- PDF exports: Decktape 1440x1024 light mode --media print. iLovePDF para compresión.
- Guión presentación: GUION_PRESENTACION.md + .html (light mode).
- **Commits:** `7e40bbf`, `24d4756`

### html2pptx.py v2 — rewrite completo
- Reescrito desde cero: light palette, 10 slide type builders, clean layout.
- v2 initial: 102KB (no images), text loss in cards (only read `<p>` tags).
- **Commit:** `049d800`

### html2pptx.py v2 — fixes
- `extract_body()`: handles `<p>`, `<ul>/<li>`, recap-section (STOP/START), pilar-question.
- `add_image_safe()`: embeds bench PNGs (img/bench/) + viz PNGs (viz-*.png). 67MB output.
- Recap-keep-block (slide 02): detected and injected as first card with all 6 KEEP items.
- Orphan rec-boxes captured as highlight cards.
- Case narrative parser: domain, context (case-contexto-bajada NOT case-ctx), perfil (roles + items), agrupados, full DAR stages with refs and aplicaciones.
- Build layout: left sidebar (profile card) + right (context + 3 DAR columns).
- Known: benchmark images overlay text (python-pptx images are separate shapes, not inline).

### IDEAS.md — captura de aprendizajes
- 7 PD-Spec engine ideas (ship presentation custom, html2pptx tool, Decktape pipeline, light/dark toggle, etc.)
- 7 product/methodology ideas (síntesis por flujo, benchmark por pilares, muro invisible colaboración)
- 3 bugs/gotchas técnicos
- Insight clave: "muro invisible" — HTML outputs impiden colaboración con Camila. PPTX debe ser first-class, no afterthought.

### Pending
- Investigar HTML "ready to export" (PPTX, DOCX, PDF nativo)
