...
**Additional signal (2026-03-21):** Three architectural insights for Layer 3:
1. **Source types** need formalization — `raw`, `analysis`, `directive` — each with distinct /extract behavior.
2. **Feedback loop** — when /ship generates artifacts, emergent synthesis reveals new insights trapped in 03_Outputs/. Orchestrator should monitor and propose graph updates without breaking unidirectional invariant. Was scoped as "Bidirectional pipeline (F)" in Wave 3.5.
3. **Evidence vs Context model** — not all inputs should build the graph. Evidence (your users, your business, your constraints) → generates [IG-XX]. Context (competitive benchmarks, industry trends, historical artifacts) → informs agent reasoning but produces no claims. The agent should know the difference and use context for cross-references and amplification, not for building insights. Example: a competitive benchmark doesn't tell you users need X — users tell you that. The benchmark tells you how a competitor solved X.
